...
│
├── data/
│   ├── game/                     # Raw per-sensor tracking files (matches)
│   ├── practice/                 # Raw per-sensor tracking files (training)
│   ├── summary.csv               # Aggregated player × session KPIs (main data source)
│   ├── overall_practice.pdf      # Practice overview report
│   └── overall_game.pdf          # Match overview report
│
├── results/
│   ├── Images/                   # Exported Python visualisations
│   └── Images_R/                 # Exported R visualisations
│
├── sources/
│   └── sources.pdf               # Scientific references and AI assistance disclosure
│
...
├── requirements.txt              # pip dependencies (Python)
├── renv.lock                     # R package versions snapshot (renv)
├── renv/activate.R               # renv auto-loader
├── .Rprofile                     # renv hook
...
| Raw tracking files (`game/`, `practice/`) | Per-sensor position and speed time series. Available for within-session analysis (not exploited in the current version). |
...
conda activate football-monitoring
...
The `environment.yml` and `requirements.txt` files pin compatible version ranges for all dependencies. DuckDB in particular has breaking changes between major versions — the upper bounds protect against unexpected behaviour.

> **Note:** All file paths in the notebook are relative. Do not use absolute paths. `football.duckdb` is created automatically on first run — do not commit it.

---

## R analysis (`main.Rmd`)

**Stack:** readr · dplyr · tidyr · ggplot2 · scales · coin · rstatix · knitr · kableExtra · broom
...
This project uses [**renv**](https://rstudio.github.io/renv/) to ensure reproducible R package versions. The `renv.lock` file records the exact version of every package used.

**Step 1 — Restore the package environment:**
```r
install.packages("renv")   # only needed if renv is not already installed
renv::restore()             # installs all packages at the recorded versions
```

**Step 2 — Knit the report:**
```r
# In RStudio: click Knit (Ctrl+Shift+K)
# Or from the console:
rmarkdown::render("main.Rmd")
```

This produces `bendekoum.celine.html` in the project root.

> If you add new packages, run `renv::snapshot()` before committing to keep `renv.lock` up to date.  
> The R script reads `data/summary.csv` with relative paths and applies the same quality control thresholds as the Python pipeline to ensure consistent results across both analyses.
...
- `football.duckdb` is generated locally on first run and excluded from Git
- Python dependencies are managed via `environment.yml` (Conda) or `requirements.txt` (pip)
- R dependencies are managed via `renv` — run `renv::restore()` to replicate the exact environment
- Quality control rules are defined once in the DuckDB view (Python) and mirrored in `main.Rmd` (R), guaranteeing consistent results across both pipelines
